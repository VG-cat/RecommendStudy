...
- 再次创建外部表 student2
  ...
  由于文件受hdfs管理，可以直接将文件上传到指定目录'/tmp/student'下
...
- Sqoop是一款进行数据传送的工具，可以在Hadoop的hdfs和关系数据库之间传输数据
- 可以使用sqoop把数据从MySQL或者Oracle导入到hdfs中，也可以把数据从hdfs导入到MySQL或者Oracle中
- 可以自动执行数据传输中的大部分过程，使用mapreduce导入和导出数据，提供并行操作和容错
- 提供了多种数据传输的方式
...
![](./img/Snipaste_2025-02-15_19-40-14.png)

安装:

### **1. 前置条件**

在安装 Sqoop 之前，确保满足以下条件：

1. **Hadoop**：Sqoop 依赖于 Hadoop，因此需要先安装并配置 Hadoop。
2. **Java**：确保已安装 Java（推荐 JDK 1.8 或更高版本）。
3. **数据库驱动**：根据使用的数据库，下载对应的 JDBC 驱动（如 MySQL 的 `mysql-connector-java.jar`）。

### **2. 下载 Sqoop**

1. 访问 Sqoop 的官方下载页面：
   [Apache Sqoop Downloads](https://sqoop.apache.org/docs/)

### **3. 解压 Sqoop**

将下载的 Sqoop 压缩包解压到目标目录：

```
tar -zxvf sqoop-1.4.7-cdh5.7.0.tar.gz -C /opt/
```

### **4. 配置环境变量**

编辑 `~/.bashrc` 或 `/etc/profile` 文件，添加 Sqoop 的环境变量：

```
export SQOOP_HOME=/opt/sqoop-1.4.7-cdh5.7.0
...
1. **复制 Hadoop 配置文件**：
   将 Hadoop 的配置文件（如 `core-site.xml`、`hdfs-site.xml`、`yarn-site.xml` 等）复制到 Sqoop 的 `conf` 目录：

   ```
   cp $HADOOP_HOME/etc/hadoop/*.xml $SQOOP_HOME/conf/
   ```

2. **配置数据库驱动**：
   将数据库的 JDBC 驱动（如 `mysql-connector-java.jar`）复制到 Sqoop 的 `lib` 目录：

   

   ```
   cp /path/to/mysql-connector-java.jar $SQOOP_HOME/lib/
   ```

3. **配置 Sqoop 环境**：
   编辑 `$SQOOP_HOME/conf/sqoop-env.sh` 文件，设置 Hadoop 和 Hive 的相关路径：

   

   ```
   export HADOOP_COMMON_HOME=/home/hadoop/app/hadoop-2.6.0-cdh.5.7.0
...
运行以下命令，验证 Sqoop 是否安装成功：

```
sqoop version
```

如果安装成功，会显示 Sqoop 的版本信息。
...
1. 确保 MySQL 服务已启动，并且可以访问。

2. 运行以下命令，将 MySQL 表数据导入到 HDFS：
   ...
   - `--password`：数据库密码。
   - `--table`：要导入的表名。
   - `--target-dir`：HDFS 目标目录。默认存放到user/username目录下
   - `--m`：并行任务数。

3. 检查 HDFS 中是否生成了数据：

   ```
   hdfs dfs -ls /user/hadoop/table_name
   ```

#### **从 HDFS 导出数据到 MySQL**

1. 运行以下命令，将 HDFS 数据导出到 MySQL 表：
...
<tr>
<td> 1 </td>
<td> FILE INFO:name:file1.txt
FILE INFO:type:txt
FILE INFO:size:1024</td>
<td>path:/home/pics
creator:Jerry
</td>
</tr>
<tr>
<td> 2 </td>
<td>FILE INFO:name:file2.jpg
FILE INFO:type:jpg
FILE INFO:size:5032</td>
<td> SAVE INFO:path:/home
SAVE INFO:creator:Tom</td>
</tr>
</table>
...
- HBase可以存储超大数据并适合用来进行大数据的`实时`查询
